...
def home_actions():
    ...
    if pressed == 'comment':
        ...
        cmd.execute("INSERT INTO COMMENT VALUES(NULL, '" + lid + "', '" + uid + "', '" + comment + "', CURDATE(), CURTIME())")
...
def adminUserList():
    if request.method == 'POST':
        ...
        cmd.execute("SELECT `login`.`lid`,`username`, `users`.`name`,`photo` FROM `users`,`login` WHERE \
        `login`.`lid`=`users`.`lid` AND `login`.`lid`!=1 ORDER BY `login`.`username`")
        result = cmd.fetchall()
...
def removePost(uid):
    ...
    cmd.execute("SELECT `count` FROM `uploads` WHERE `upload_id`='"+uid+"' ")
    count = cmd.fetchone()[0]
    if str(count) == '10101':
        cmd.execute("SELECT `filename` FROM `upload_files` WHERE `uid`='"+uid+"' ")
        filename = cmd.fetchone()[0]
        os.remove("static/uploads/video/" + filename)
    else:
        cmd.execute("SELECT `filename` FROM `upload_files` WHERE `uid`='"+uid+"' ")
        files = cmd.fetchall()
        for filename in files:
            os.remove("static/uploads/" + filename[0])
    ...
    cmd.execute("DELETE FROM `reports` WHERE `upload_id`='"+uid+"' ")
    con.commit()
    print("deleted")
    return redirect(request.referrer)
...
def removeAcc(lid):
    ...
    for i in res:
        if str(i[1]) == '10101':
            os.remove("static/uploads/video/" + i[0])
        else:
